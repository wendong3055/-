...
export async function rhRequest(key:string,path:string,body:Record<string,unknown>|FormData,billable=false,rawAuth=false){
  let r:Response;
  try{r=await fetch(RH_CREATOR_ORIGIN+path,{method:'POST',redirect:'error',signal:AbortSignal.timeout(45000),headers:{authorization:rawAuth?key:`Bearer ${key}`,...(body instanceof FormData?{}:{'content-type':'application/json'})},body:body instanceof FormData?body:JSON.stringify(body)});}catch{throw new CreatorError(billable?'提交结果尚未确认，请在 RunningHub 核对，勿重复生成。':'连接 RunningHub 暂时失败，请稍后重试。',billable);}
...
export async function creatorApp(key:string,id:string):Promise<RhModel>{
  ...
  let p:Record<string,any>;
  try{const u=new URL('/api/webapp/apiCallDemo',RH_CREATOR_ORIGIN);u.searchParams.set('apiKey',key);u.searchParams.set('webappId',id);const r=await fetch(u,{redirect:'error',signal:AbortSignal.timeout(25000)});if(!r.ok)throw new Error();p=JSON.parse(new TextDecoder().decode(await limitedBytes(r,1024*1024)));}catch{throw new CreatorError('应用参数读取失败，请确认应用链接和 Key 权限。');}
...
export async function archiveCreatorResult(owner:string,id:string,index:number,source:string,kind:string){
  ...
  const r=await fetch(u,{redirect:'error',signal:AbortSignal.timeout(90000)});
