...
export function RunningHubSettings({ config, onRefresh, region, busy, member = false }: { config: Config | null; onRefresh: () => void; region: 'cn' | 'international'; busy: boolean; member?: boolean }) {
  ...
  return <details className="rh-connection">
    <summary><span className="rh-monogram" aria-hidden="true">RH</span><span><strong>RunningHub 设置</strong><small>{region === 'cn' ? '中国站 · runninghub.cn' : '国际站 · runninghub.ai'}</small></span><span className={configured ? 'rh-configured' : 'rh-unconfigured'}>{config ? configured ? '密钥已配置' : '点击配置密钥' : '检查配置中'}</span></summary>
