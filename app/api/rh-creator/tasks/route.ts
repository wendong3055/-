...
export async function POST(request:Request){const owner=await generationOwner(request);if(!owner)return json({error:'请先登录。'},401);let id='',inserted=false,submitted=false;try{
  ...
  await creatorTasks(owner);const key=await creatorKey(owner),endpoint=String(form.get('endpoint')||'');const app=endpoint.startsWith('app:');const model=app?await creatorApp(key,endpoint.slice(4)):creatorModel(endpoint);if(!model)return json({error:'模型不存在。'},400);
  ...
  const payload=validateParams(model,raw,counts);await insertCreator({id,owner,name:model.name_cn,endpoint,kind:model.output_type,inputs:{values:payload,fingerprint:model.fingerprint}});inserted=true;
  for(const p of model.params.filter(mediaParam)){const values=[...urls[p.key]];for(const file of files[p.key])values.push(await creatorUpload(key,file,app));if(values.length)payload[p.key]=p.multiple?values:values[0];else if(app)payload[p.key]='';}
  ...
  const r=await rhRequest(key,app?'/task/openapi/ai-app/run':`/openapi/v2/${endpoint}`,body,true);const remote=app?r.data?.taskId:r.taskId;
