...
export async function POST(request: Request) {
  const user = await getChatGPTUser();
  const ownerId = user?.userId ?? 'local-preview';
  const body = await request.json() as { artworkId?: string; artworkName?: string; frameId?: string; frameName?: string; name?: string; sizeCount?: number };
  ...
  await getDb().insert(jobs).values([
    ...
    { id: crypto.randomUUID(), ownerId, productId: id, kind: 'single_sizes', status: 'waiting_for_sample', version: 'v1', outputCount: sizeCount, createdAt: now },
