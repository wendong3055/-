...
export async function POST(r:Request,c:{params:Promise<{id:string}>}) {
  ...
  try {const {row,spec,config,workspace}=await productionContext(owner,(await c.params).id);let b:{generationId:string;review:string;note:string;action?:string;marks?:unknown;sceneGenerationId?:string};
    try{b=await productionJson(r,4000) as typeof b;}catch(e){throw new ProductionError(e instanceof Error?e.message:'验收内容无效。');}
    const item=workspace.plans.find(p=>p.id===row.plan_id)!.items.find(i=>i.id===row.id)!;
    if(b?.action==='save-marks'&&preservesSourceSizeMarks(item))throw new ProductionError('本图已保留原框架尺寸标注，不再另加箭头；标注不正确时请标记重做。');
    if(b?.action==='reuse-scene'||b?.action==='save-marks'){await updateSizeLayout(owner,row.id,b);return NextResponse.json({ok:true});}
    if(b?.review==='accepted'&&row.kind==='size'&&config.sceneTitle&&!preservesSourceSizeMarks(item)&&!validSizeMarks(spec?.marks,row.generation_id,!!spec?.depthCm))throw new ProductionError('请先在尺寸图上保存准确标注，再验收。');
